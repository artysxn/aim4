...
export function deltaLevel(value, peer, bands) {
  if (!Number.isFinite(value) || !Number.isFinite(peer) || !bands) return 0;
  const d = value - peer;
...
function iconSvg(key) {
  const path = SVG[key];
  if (!path) return '';
  return `<svg class="pf-delta-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" width="14" height="14" fill="currentColor" aria-hidden="true">${path}</svg>`;
}
...
export function deltaMarkHtml(level) {
  ...
  return `<span class="pf-delta is-${dir}" aria-hidden="true">${iconSvg(key)}</span>`;
}

/** Number HTML plus the mark, when both value and peer are finite. */
export function withDeltaHtml(text, value, peer, bands) {
  return `${text}${deltaMarkHtml(deltaLevel(value, peer, bands))}`;
}
