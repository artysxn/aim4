// ---------------------------------------------------------------------------
// src/cs3d/projectilesCore.js
// Thrown grenades in the world, for BOTH renderers.
//
// One implementation, two bodies — the same arrangement movement has with
// shared/sim3d/motion.js and shooting has with shared/sim3d/recoil.js. The
// physics is shared/sim3d/grenade.js and knows nothing about three.js; this
// file is the entity around it, and the only thing it needs from a renderer is
// a Group, a Vector3 and a Line. Both three builds have all three, identically.
//
// So the class is built by a factory that is HANDED its three namespace:
//
//   src/cs3d/projectiles.js        binds it to `three/webgpu` (the explorer)
//   src/weapons/trainerNadeFlight.js  binds it to `three` (the aim trainer)
//
// which is the only way one file can serve both. A module can only ever say
// `from 'three'` OR `from 'three/webgpu'`, and whichever it said would put a
// whole second 1.2 MB three core on the other page. Injecting the namespace
// costs a closure and duplicates nothing.
//
...
/**
 * @param {object} deps
 * @param {object} deps.THREE          the renderer's three namespace
 * @param {(o: object) => object} [deps.cloneSkinned]  SkeletonUtils clone, bound
 *   to the same three; omit and thrown grenades fly without a model
 * @param {(collider, audience, movers) => object} deps.createHullWorld
 * @returns {{ Projectiles: Function }}
 */
export function makeProjectiles({ THREE, cloneSkinned = null, createHullWorld }) {
  ...
  class Projectiles {
    ...
    /**
     * The world directly, for a caller that already has one.
     *
     * The aim trainer's arenas are a box list rather than a BVH, and their
     * tracer is built from the boxes (src/utils/simWorld.js) — so there is no
     * collider to hand `setCollider`, only the world it would have produced.
     */
    setWorld(world) {
