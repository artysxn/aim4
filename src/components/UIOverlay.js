    ...
    return [
      ...
      },
      {
        ...
        body: `
          ...
          <div class="settings-subhead">Bots</div>
          ${classicBotModelField()}
          <p class="muted" id="set-bots-agent-status"></p>
      ...
      },
      {
        ...
        body: `
          ...
          <label class="field-check"><input type="checkbox" id="set-cs2-ballistics" /> CS2 spray pattern and inaccuracy</label>
          <label class="field-check"><input type="checkbox" id="set-vm-aimpunch" /> Aimpunch (view-punch recoil)</label>
          <p class="muted">The game's own recoil table, generated from the weapon's seed, plus its accuracy model and fire rate. Off falls back to the trainer's older hand-authored pattern &mdash; scores from the two are not comparable.</p>
          <div class="field field-plain">
...
${botDifficultyField('set-peekswitchbots-bot-difficulty')}
  ...
  _bindSettings() {
    ...
    $('#set-cs2-ballistics')?.addEventListener('change', (e) => {
      draft((d) => { d.weapon.cs2Ballistics = e.target.checked; });
    });
    ...
    // On the whole overlay, not just the scenario drawer: the same two
    // checkboxes are rendered in Graphics as well, because a per-gamemode
    // drawer is not where anyone looks for "which bot model do I get".
  ...
  _populateSettings() {
    ...
    const cs2b = $('#set-cs2-ballistics');
    if (cs2b) cs2b.checked = s.weapon?.cs2Ballistics !== false;
    ...
    // ...and say what the pack is actually doing. Ticked-but-not-loaded is
    // otherwise indistinguishable from ticked-and-ignored, which is exactly
    // what a slow or blocked CDN looks like from the settings panel.
    const agentStatus = $('#set-bots-agent-status');
    if (agentStatus) {
      const models = sharedAgentModels();
      agentStatus.textContent = !agentBot
        ? 'Off — bots use the built-in skeletal model.'
        : classicBot
          ? 'Overridden by the classic bot model below.'
          : models.ready
            ? 'CS2 agent models loaded.'
            : models.failed
              ? `Could not load the CS2 models (${models.failed.message || models.failed}) — bots fall back to the built-in one.`
              : 'Loading the CS2 models…';
    }
