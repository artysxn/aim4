 ...
 * @returns {{vocab: string[], probs(obs: number[]): Map<string, number>}}
 */
export function loadPolicy(json) {
  if (!json || json.v !== POLICY_VERSION) {
    throw new Error(`policy: version ${json?.v} is not ${POLICY_VERSION}`);
  }
  ...
  if (layers[0].W[0].length !== OBSERVATION_SIZE) {
    throw new Error(
      `policy: first layer reads ${layers[0].W[0].length} floats, observations carry ${OBSERVATION_SIZE}`
    );
  }
